...
int main() {
    ...
    int a, b;
    unsigned long c; 
    int num_of_idragogeia = 500, solines = 1000;
    ...
    while (getline(file, line)) { // Read each line from the file
        ...
        if (iss>>a>>b>>c) { // Extract the three numbers from the line
            ...
            if (iss >> num_of_idragogeia >> solines) {
                //cout << "Read: " << num_of_idragogeia << " " << solines << endl;
                //Restriction num_of_idragogeia, solines
                if(num_of_idragogeia < 1 || num_of_idragogeia > 500) cout<<"polla idragogeia!"<<endl;
                if(solines < 1 || solines > 1000) cout<<"polloi solines!"<<endl;
            } 
            else {
    ...
    /*for (int i =  0; i <= num_of_idragogeia; i++){
        cout<<"Idragogeio noumero: "<<residualGraph[i].get_name()<<endl;
        residualGraph[i].print_edges();
    }*/
